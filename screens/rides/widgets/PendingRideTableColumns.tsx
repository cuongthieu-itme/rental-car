...
import { MoreHorizontal, CheckCircle, X, Eye } from "lucide-react";
...
export type PendingRideTableTypes = {
  id: string;
  userId: string | null;
  carId: string | null;
...
const PendingRideActions = ({ row }: { row: Row<PendingRideTableTypes> }) => {
  const ride = row.original;
  const { onOpen: onView } = useViewRide();
  const { onOpen: onApprove } = useApproveRide();
  const { onOpen: onReject } = useRejectRide();
...
export const pendingColumns: ColumnDef<PendingRideTableTypes>[] = [
  {
    accessorKey: "id",
    header: "#",
